...
struct Dice { //VB0 = 변수 하나, VAO = 배열 -> 요런 느끰
	...
	Dice(int x, int y) {
		...
		m_move.z = 0.0f;
		R_matrix = glm::rotate(R_matrix, glm::radians(-30.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		R_matrix = glm::rotate(R_matrix, glm::radians(30.0f), glm::vec3(1.0f, 0.0f, 0.0f));
...
struct Line {
	...
	Line(int x, int y) {
		...
		m_vertex[3].x = 0.0f;
		m_vertex[3].y = 1.0f;
		m_vertex[2].z = m_vertex[3].z = 0.0f;
		m_vertex[4].x = 0.5f;
		m_vertex[5].x = -0.5f;
		m_vertex[4].y = 1.0f;
		m_vertex[5].y = -1.0f;
		m_vertex[4].z = m_vertex[5].z = 0.0f;
		SetVBO();
	}
	void SetVBO() {
		glGenBuffers(1, &m_VBOVertex);
	...
	void Draw() {
		...
		int vColorLocation = glGetUniformLocation(shaderProgramID, "vColor");
		glLineWidth(5.0f);
		glUniform3f(vColorLocation, 1, 0, 0);
		glDrawArrays(GL_LINES, 0, 2);
		glUniform3f(vColorLocation, 0, 1, 0);
		glDrawArrays(GL_LINES, 2, 2);
		glUniform3f(vColorLocation, 0, 0, 1);

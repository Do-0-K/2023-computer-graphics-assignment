...
struct solar_system { //VB0 = 변수 하나, VAO = 배열 -> 요런 느끰
	glm::vec3 m_color[1];
...
vector<solar_system> g_solar_systems;
...
struct light { //VB0 = 변수 하나, VAO = 배열 -> 요런 느끰
	...
	light() {
		...
		m_move.w = 1;
		S_matrix = glm::scale(S_matrix, glm::vec3(0.3f, 0.3f, 0.3f));
		T_matrix = glm::translate(T_matrix, glm::vec3(0.0f, 0.0f, 1.5f));
		SetVBO();
	}
	void SetVBO() {
		//vbo객체 생성
...
void main(int argc, char** argv) {//--- 윈도우 출력하고 콜백함수 설정
	...
	g_solar_systems.emplace_back(0);
	g_solar_systems.emplace_back(1);
	g_solar_systems.emplace_back(2);
...
GLvoid DrawScene() { //--- 콜백 함수: 그리기 콜백 함수
	...
	glm::vec3 cameraPos = glm::vec3(3.0f, 5.0f, 5.0f); //--- 카메라 위치
	...
	glEnable(GL_CCW);
	g_lights[0].Draw();
	g_solar_systems[0].Draw();
	g_solar_systems[1].Draw();
	g_solar_systems[2].Draw();

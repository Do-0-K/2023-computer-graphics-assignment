...
bool g_Sp = false, g_BBul = false, g_Dice = false;
int g_kind = 0;
...
std::vector<glm::vec3> BB_vertex; //점
...
struct OneBBul { //VB0 = 변수 하나, VAO = 배열 -> 요런 느끰
	glm::vec3 m_color[1];
	GLuint m_VBOVertex;
	GLuint m_VBOColor;
	glm::mat4 S_matrix = glm::mat4(1.0f);
	...
	OneBBul() {
		...
		m_color[0].b = b;
		S_matrix = glm::scale(S_matrix, glm::vec3(0.8f, 0.8f, 0.8f));
		T_matrix = glm::translate(T_matrix, glm::vec3(1.0f, 0.0f, -0.5f));
		SetVBO();
	}
	void NewOneBBul() {
		SetVBO();
	...
	void Draw() {
		glBindBuffer(GL_ARRAY_BUFFER, m_VBOVertex);
		glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * BB_vertex.size(), &BB_vertex[0], GL_STATIC_DRAW);
		int positionAttrib = glGetAttribLocation(shaderProgramID, "vPos");
		//어떤 어트리브인지, 세이더에서 vec3면 3(변수 갯수), 데이터 타입, 정규화, 하나의 덩어리 크기?, 시작 위치
		glVertexAttribPointer(positionAttrib, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), 0);
		glEnableVertexAttribArray(positionAttrib);
		...
		glDrawArrays(GL_LINE_LOOP, 0, BB_vertex.size());
	}
};
vector<OneBBul> g_OneBBul;
...
struct Sphere { //VB0 = 변수 하나, VAO = 배열 -> 요런 느끰
	...
	Sphere() {
		...
		m_color[0].g = g;
		m_color[0].b = b;
		S_matrix = glm::scale(S_matrix, glm::vec3(0.15f, 0.15f, 0.15f));
	...
	void Draw() {
		...
		int vColorLocation = glGetUniformLocation(shaderProgramID, "vColor");
		glUniform3f(vColorLocation, m_color[0].r, m_color[0].g, m_color[0].b);
		glDrawArrays(GL_LINE_LOOP, 0, GU_vertex.size());
	}
};
...
void main(int argc, char** argv) {//--- 윈도우 출력하고 콜백함수 설정
	...
	ReadDObj("cone.obj", BB_vertex);
	g_OneBBul.emplace_back();
	ReadDObj("sphere.obj", GU_vertex);
	g_Sphere.emplace_back();
...
GLvoid Keyboard(unsigned char key, int x, int y) {
	...
	case 's':
		...
		g_OneBBul[0].R_matrix = glm::mat4(1.0f);
		g_Sphere[0].R_matrix = glm::mat4(1.0f);
		g_Dices[0].R_matrix = glm::mat4(1.0f);
	...
	case 'c':
		random_device rd;
		default_random_engine dre(rd());
		uniform_int_distribution<int> kind(0, 2);
		g_kind = kind(dre);
		switch (g_kind) {
		case 0:
			g_Dices[0].NewDice();
			...
			g_OneBBul[0].NewOneBBul();
			g_OneBBul[0].T_matrix = glm::mat4(1.0f);
			g_OneBBul[0].T_matrix = glm::translate(g_OneBBul[0].T_matrix, glm::vec3(1.0f, 0.0f, -0.5f));
			break;
		case 1:
			g_Dices[0].NewDice();
			g_Dices[0].T_matrix = glm::mat4(1.0f);
			g_Dices[0].T_matrix = glm::translate(g_Dices[0].T_matrix, glm::vec3(-2.0f, 0.0f, -0.25f));
			g_Sphere[0].NewSphere();
			g_Sphere[0].T_matrix = glm::mat4(1.0f);
			g_Sphere[0].T_matrix = glm::translate(g_Sphere[0].T_matrix, glm::vec3(1.0f, 0.0f, -0.25f));
			break;
		case 2:
			g_OneBBul[0].NewOneBBul();
			g_OneBBul[0].T_matrix = glm::mat4(1.0f);
			g_OneBBul[0].T_matrix = glm::translate(g_OneBBul[0].T_matrix, glm::vec3(-2.0f, 0.0f, -0.5f));
			g_Sphere[0].NewSphere();
			g_Sphere[0].T_matrix = glm::mat4(1.0f);
			g_Sphere[0].T_matrix = glm::translate(g_Sphere[0].T_matrix, glm::vec3(1.0f, 0.0f, -0.25f));
			break;
	...
	case 1:
		if (g_isOn) {
			if (g_Rotate == 1) {
				...
				if (g_BBul) {
					g_OneBBul[0].R_matrix = glm::rotate(g_OneBBul[0].R_matrix, glm::radians(-3.0f), glm::vec3(1.0f, 0.0f, 0.0f));
				}
				if (g_Sp) {
					g_Sphere[0].R_matrix = glm::rotate(g_Sphere[0].R_matrix, glm::radians(-3.0f), glm::vec3(1.0f, 0.0f, 0.0f));
				}
			}
			if (g_Rotate == 2) {
				if (g_Dice) {
	...
	case 2:
		if (g_isOn1) {
			if (g_Rotate1 == 1) {
				...
				if (g_BBul) {
					g_OneBBul[0].R_matrix = glm::rotate(g_OneBBul[0].R_matrix, glm::radians(-3.0f), glm::vec3(0.0f, 1.0f, 0.0f));
				}
				if (g_Sp) {
					g_Sphere[0].R_matrix = glm::rotate(g_Sphere[0].R_matrix, glm::radians(-3.0f), glm::vec3(0.0f, 1.0f, 0.0f));
				}
			}
			if (g_Rotate1 == 2) {
				if (g_Dice) {
